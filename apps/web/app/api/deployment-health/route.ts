import { backendUpstreamHeaders } from "@/lib/server-auth";
...
export async function GET() {
  if (!backend) {
    return Response.json(
      { status: "degraded", frontend: "ready", backend: "not_configured", missing: ["BACKEND_API_URL"] },
  ...
  try {
    ...
    const [readyResponse, healthResponse] = await Promise.all([
      fetch(`${backend}/health/ready`, { cache: "no-store", signal: AbortSignal.timeout(5000), headers }),
      fetch(`${backend}/health`, { cache: "no-store", signal: AbortSignal.timeout(5000), headers }).catch(() => null),
    ]);
    ...
    return Response.json(
      {
        status: readyResponse.ok ? "ready" : "degraded",
        frontend: "ready",
        backend: readyResponse.ok ? "ready" : "not_ready",
        backendHealth: health,
      },
      { status: readyResponse.ok ? 200 : 503, headers: { "Cache-Control": "no-store" } },
    );
  } catch (error) {
    console.error("[deployment-health] backend unreachable", error instanceof Error ? error.message : String(error));
    return Response.json(
      { status: "degraded", frontend: "ready", backend: "unreachable" },

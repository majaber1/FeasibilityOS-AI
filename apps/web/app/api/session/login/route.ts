...
import { backendUpstreamHeaders, backendUrl, isSafeBrowserMutation, SESSION_COOKIE, sessionCookieOptions } from "@/lib/server-auth";
...
export async function POST(request: NextRequest) {
  ...
  const upstream = await fetch(backendUrl("auth/login"), {
    method: "POST",
    headers: backendUpstreamHeaders({ "content-type": "application/json" }),
    body,
    cache: "no-store",
  });

...
import { createProject, getToken, listProjects, listStudies, listV2Studies, setProjectArchived, updateProject, v2StudyPrimaryAction, type Project, type Study, type V2StudySummary } from "@/lib/api";
...
export default function ProjectsPage() {
  ...
  async function load(archived = showArchived) {
    ...
    try {
      ...
      const [groups, v2Studies] = await Promise.all([
        ...
        listV2Studies(token).catch(() => [] as V2StudySummary[]),
      ]);
      ...
      for (const project of rows) {
        const match = v2Studies.find((s) => String(s.project_id ?? "") === String(project.id));
        if (match) v2Map[project.id] = match;
      }
  ...
  async function submit(e: React.FormEvent) {
    ...
    try {
      if (editing) {
        await updateProject(token, editing.id, { name, industry, investment, stage });
        reset();
        await load();
      } else {
        const created = await createProject(token, { name, industry, investment, stage });
  ...
  return <main className="container-page py-12" data-testid="projects-workspace"><div className="flex flex-wrap items-end justify-between gap-4"><div><h1 className="text-3xl font-bold text-ink-900">{c.title}</h1><p className="mt-2 text-ink-600">{c.intro}</p></div><button onClick={() => setShowForm(true)} data-testid="add-project-btn" className="rounded-lg bg-brand-600 px-5 py-2.5 font-medium text-white">{c.add}</button></div>
    ...
    {projects === null ? <p className="py-12 text-center">{c.loading}</p> : projects.length === 0 ? <section className="mt-8 rounded-2xl border border-dashed border-slate-300 bg-slate-50 p-10 text-center"><h2 className="text-xl font-semibold">{c.empty}</h2><p className="mt-2 text-ink-600">{c.emptyBody}</p><button onClick={() => setShowForm(true)} data-testid="add-project-empty" className="mt-5 rounded-lg bg-brand-600 px-5 py-2.5 text-white">{c.add}</button></section> : <div className="mt-8 grid gap-4 md:grid-cols-2">{projects.map((p) => {
      ...
      const v2 = v2ByProject[p.id];
      const actionKey = v2 ? v2StudyPrimaryAction(v2.phase) : "startStudy";
      const primaryLabel = c[actionKey];
      const v2Href = v2
        ? `/projects/${p.id}/studies/${v2.study_id}/workspace`
        : `/projects/${p.id}/studies/new/workspace`;
        ...
        <article key={p.id} data-testid={`project-card-${p.id}`} className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
          ...
          <div className="mt-5 flex flex-wrap gap-2">
            {!p.is_archived && (
              <Link
                href={v2Href}
                data-testid={`ai-study-${p.id}`}
                data-phase={v2?.phase ?? "none"}
            ...
            {!p.is_archived && (v2 || study) && (
              <Link href={v2 ? `${v2Href}?panel=funding` : `${legacyHref}?tab=funding`} className="rounded-lg border px-3 py-2 text-sm">{c.funding}</Link>
            )}
            {!p.is_archived && (v2 || study) && (
              <Link href={v2 ? `${v2Href}?panel=report` : `${legacyHref}?tab=report`} className="rounded-lg border px-3 py-2 text-sm">{c.report}</Link>
            )}

...
export default function FinancialAnalysisTab({
  ...
  return <div className="mt-5 space-y-4" data-testid="financial-analysis-workspace">
    ...
    {missing === null ? <p>{ar ? "جارٍ تحميل الافتراضات..." : "Loading assumptions..."}</p> : missing.length > 0 ? <p data-testid="missing-assumptions">{ar ? "أضف الافتراضات المطلوبة:" : "Add required assumptions:"} {missing.join(", ")}</p> : null}
    {onOpenAssumptions ? (
      <button type="button" onClick={onOpenAssumptions} data-testid="open-assumptions-from-financial" className="rounded-lg border border-slate-300 px-4 py-2 text-sm">
    ...
    {study.result && <div data-testid="financial-analysis-result" className="space-y-2 rounded-xl bg-slate-50 p-4">
      ...
      {([ ["NPV (SAR)", study.result.npv], ["ROI (%)", study.result.roi_percent], ["IRR (%)", study.result.irr_percent], [ar ? "الاسترداد (سنوات)" : "Payback (years)", study.result.payback_years] ] as const).map(([label, value]) => <p key={label} data-testid={`metric-${label}`}>{label}: {value == null ? "UNKNOWN" : new Intl.NumberFormat(locale, { maximumFractionDigits: 2 }).format(value)}</p>)}

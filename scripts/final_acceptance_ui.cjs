...
async function captureStudy(browser, study) {
  ...
  try {
    ...
    result.gates.verdict_visible =
      /NO_GO|NO-GO|DEFER|GO_WITH|GO\b|لا تمضي|تأجيل|verdict|القرار|INSUFFICIENT/i.test(body) ||
    ...
    result.gates.refresh_persistence =
      /NO_GO|DEFER|GO|NPV|IRR|risk|INSUFFICIENT/i.test(afterRefresh) &&
      afterRefresh.length > 200;
    ...
    result.gates.reopen_persistence =
      /NO_GO|DEFER|GO|NPV|IRR|risk|INSUFFICIENT/i.test(afterReopen) &&
      afterReopen.length > 200;
